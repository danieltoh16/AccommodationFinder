...
$sql = "SELECT * FROM residence WHERE resName LIKE '$key' OR address LIKE '$key' OR resID LIKE '$key'";
$check = $conn -> query($sql);

$wrongMsg = "No results found, redirecting back to search page";

if ($check->num_rows > 0){
    echo("<SCRIPT type=\"text/javascript\">window.location = 'accommodationList.php?id=$searchKey';</SCRIPT>");
    session_start();
